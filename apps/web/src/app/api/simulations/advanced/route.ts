...
async function readBoundedJson(request: Request): Promise<unknown> {
  ...
  if (Number.isFinite(declaredLength) && declaredLength > MAX_REQUEST_BYTES) {
    throw new ApplicationError('validation_failed', 'Request body exceeds 2 MB.');
  }
...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  try {
    ...
    const result = await runGovernedAdvancedSimulation(getDatabasePool(), principal, body as never);
    return NextResponse.json(result, {
      status: result.state === 'queued' ? 202 : 201,
    ...
    return NextResponse.json(toPublicError(error, requestId), {
      ...
      headers: {
        ...
        ...(error instanceof ApplicationError && error.retryAfterSeconds ? { 'Retry-After': String(error.retryAfterSeconds) } : {}),
      },
    });
  }
}
